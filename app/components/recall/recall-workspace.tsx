    ...
    <section className="recall-workspace" aria-label="导入交易回忆复盘工作区">
      <header className="recall-header">
        <div className="recall-heading">
          <span className="eyebrow">导入交易 · 回忆复盘</span>
      ...
      <div className={`recall-layout${leftNavOpen ? "" : " nav-collapsed"}`}>
        ...
        <div className="recall-main">
          ...
          <div className="recall-position-strip">
            ...
            <div className="recall-position-stats"><span>持仓 <b>{position.quantity}</b></span><span>均价 <b>{Number(position.averageCost).toFixed(2)}</b></span><span>估值 <b>{latestCandle?.close?.toFixed(2) ?? "—"}</b></span><span className={Number(position.netPnl) >= 0 ? "positive" : "negative"}>净盈亏 <b>{money(position.netPnl, instrument.currency)}</b></span></div>
          </div>
          ...
          {statsOpen && <aside className="recall-stats-panel" aria-label="当前统计"><div><strong>当前统计</strong><button type="button" aria-label="关闭统计" onClick={() => setStatsOpen(false)}><X size={15} /></button></div><dl><dt>标记收盘价</dt><dd>{latestCandle?.close?.toFixed(2) ?? "—"}</dd><dt>标记时间</dt><dd>{latestCandle ? formatMarketCursor(latestCandle.time, instrument.market) : "—"}</dd><dt>已揭示成交</dt><dd>{chartExecutions.length} / {currentExecutions.length}</dd><dt>持仓数量</dt><dd>{position.quantity}</dd><dt>平均成本</dt><dd>{position.averageCost}</dd><dt>净盈亏</dt><dd className={Number(position.netPnl) >= 0 ? "positive" : "negative"}>{money(position.netPnl, instrument.currency)}</dd></dl></aside>}
