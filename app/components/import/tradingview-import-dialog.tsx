...
export function TradingViewImportDialog({file,instruments,onCancel,onConfirm}: Props) {
  const [market,setMarket] = useState<TradingViewInstrument['market']>(tradingViewInstrumentFromName(file.name)?.market ?? 'CN-SH');
  ...
  return <div className="modal-backdrop">
    <section className="import-dialog tradingview-dialog" ref={dialogRef} role="dialog" aria-modal="true" aria-label="核对模拟交易证券">
      ...
      <p>{file.name}</p>
      <p>CSV 没有证券列，请核对沪深证券市场和六位代码。名称会自动补全。</p>
      <form onSubmit={event=>{event.preventDefault();if (/^\d{6}$/.test(symbol)) onConfirm({market,symbol});}}>
        <label>证券市场
          <select aria-label="证券市场" value={market} onChange={event=>setMarket(event.target.value === 'CN-SZ' ? 'CN-SZ' : 'CN-SH')}>
            <option value="CN-SH">上海证券交易所</option>
            <option value="CN-SZ">深圳证券交易所</option>
          </select>
        </label>
        <label>证券代码
          <input aria-label="证券代码" list="simulation-instruments" inputMode="numeric" pattern="[0-9]{6}" required maxLength={6} value={symbol} onChange={event=>setSymbol(event.target.value.trim())} />
        </label>
        <datalist id="simulation-instruments">{instruments.filter(i=>i.market===market).map(i=><option key={i.id} value={i.symbol}>{i.name}</option>)}</datalist>
