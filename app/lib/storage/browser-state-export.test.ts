...
describe("exportLegacyBrowserState", () => {
  ...
  it("normalizes JSON-unsafe optional values from legacy IndexedDB records", async () => {
    await seedStore(DAILY_CANDLES, { instrumentId: "HK:700", tradingDate: "2025-01-02", open: "1", high: "2", low: "0.5", close: "1.5", volume: "10", currency: "HKD", provider: "tencent", providerSymbol: "700", adjustmentMode: "raw", fetchedAt: "2025-01-02T03:04:05.000Z", legacyOptional: undefined });

    const payload = await exportLegacyBrowserState({ excludeDemo: true });

    expect(payload?.dailyCandles).toEqual([expect.objectContaining({ instrumentId: "HK:700" })]);
    expect(payload?.dailyCandles[0]).not.toHaveProperty("legacyOptional");
  });
