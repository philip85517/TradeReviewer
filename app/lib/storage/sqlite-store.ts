...
function validateImportHistory(value: unknown): asserts value is ImportHistoryEntry { if (!value || typeof value !== "object" || typeof (value as ImportHistoryEntry).id !== "string" || typeof (value as ImportHistoryEntry).fileName !== "string" || typeof (value as ImportHistoryEntry).importedAt !== "string" || typeof (value as ImportHistoryEntry).tradeCount !== "number") throw new Error("Invalid import history"); }
function validateJob(value: unknown): asserts value is MarketDataJob { if (!value || typeof value !== "object" || typeof (value as MarketDataJob).instrumentId !== "string" || typeof (value as MarketDataJob).status !== "string" || !Array.isArray((value as MarketDataJob).intervals)) throw new Error("Invalid market data job"); }
...
function validateCoverage(value: unknown): asserts value is CoverageRecord { if (!value || typeof value !== "object" || typeof (value as CoverageRecord).instrumentId !== "string" || (value as CoverageRecord).adjustmentMode !== "raw") throw new Error("Invalid coverage"); }
function validateIntervalCoverage(value: unknown): asserts value is IntervalCoverageSegment & { instrumentId: string; adjustmentMode?: "raw" } { if (!value || typeof value !== "object" || typeof (value as { instrumentId?: unknown }).instrumentId !== "string" || !["15m", "1D"].includes((value as IntervalCoverageSegment).interval) || typeof (value as IntervalCoverageSegment).requestedStart !== "string" || typeof (value as IntervalCoverageSegment).requestedEnd !== "string") throw new Error("Invalid interval coverage"); }
...
export class SqliteStore {
  ...
  mergeBrowserState(payload: BrowserStatePayload): MigrationReport {
    ...
    const report = withSqliteTransaction(this.database, () => {
      ...
      for (const coverage of payload.intervalCoverage) { count(this.getIntervalCoverage().find((item) => item.instrumentId === coverage.instrumentId && item.interval === coverage.interval && item.adjustmentMode === (coverage.adjustmentMode ?? "raw")), coverage); this.putIntervalCoverage(coverage); }
  ...
  private putReviewState(state: EpisodeReviewState) { if (!state || state.version !== 2 || typeof state.episodeId !== "string" || typeof state.replayCursor !== "string" || !Array.isArray(state.drawings)) throw new Error("Invalid review state"); this.database.prepare("insert into reviews (episode_id, cursor_json, drawings_json, updated_at) values (?, ?, ?, current_timestamp) on conflict(episode_id) do update set cursor_json = excluded.cursor_json, drawings_json = excluded.drawings_json, updated_at = excluded.updated_at").run(state.episodeId, json({ replayCursor: state.replayCursor, timeframe: state.timeframe, activePanelTab: state.activePanelTab }, "review cursor"), json(state.drawings, "review drawings")); }
