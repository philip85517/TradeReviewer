...
function json(value: unknown, field: string) {
  try { return JSON.stringify(value); } catch { throw new Error(`Invalid ${field}`); }
}
...
export class SqliteStore {
  ...
  getReviews(): EpisodeReviewRecord[] { return (this.database.prepare("select review_json from reviews order by episode_id").all() as Row[]).map((row) => parseJson<EpisodeReviewRecord>(row.review_json, "review")); }
  getReview(episodeId: string): EpisodeReviewRecord | undefined { const row = this.database.prepare("select review_json from reviews where episode_id = ?").get(episodeId) as Row | undefined; return row ? parseJson<EpisodeReviewRecord>(row.review_json, "review") : undefined; }
  ...
  getDailyCandles(instrumentId?: string): DailyCandleRecord[] { return (this.database.prepare(`select d.*, p.provider, p.provider_symbol, p.metadata_json from daily_candles d left join provider_symbols p on p.instrument_id = d.instrument_id ${instrumentId ? "where d.instrument_id = ?" : ""} order by d.instrument_id, d.date`).all(...(instrumentId ? [instrumentId] : [])) as Row[]).map((row) => { const metadata = row.metadata_json ? parseJson<Record<string, unknown>>(row.metadata_json, "provider metadata") : {}; return { instrumentId: asString(row.instrument_id, "instrument id"), tradingDate: asString(row.date, "date"), open: asString(row.open, "open"), high: asString(row.high, "high"), low: asString(row.low, "low"), close: asString(row.close, "close"), volume: typeof row.volume === "string" ? row.volume : "", currency: typeof metadata.currency === "string" ? metadata.currency : "", provider: (row.provider ?? "yahoo") as DailyCandleRecord["provider"], providerSymbol: typeof row.provider_symbol === "string" ? row.provider_symbol : "", adjustmentMode: "raw", fetchedAt: typeof metadata.fetchedAt === "string" ? metadata.fetchedAt : "" }; }); }
  getMarketCandles(instrumentId?: string, interval?: MarketCandleRecord["interval"]): MarketCandleRecord[] { const clauses = [instrumentId ? "m.instrument_id = ?" : "", interval ? "m.interval = ?" : ""].filter(Boolean); const values = [instrumentId, interval].filter((value): value is string => Boolean(value)); return (this.database.prepare(`select m.*, p.provider, p.provider_symbol, p.metadata_json from market_candles m left join provider_symbols p on p.instrument_id = m.instrument_id ${clauses.length ? `where ${clauses.join(" and ")}` : ""} order by m.instrument_id, m.interval, m.timestamp`).all(...values) as Row[]).map((row) => { const metadata = row.metadata_json ? parseJson<Record<string, unknown>>(row.metadata_json, "provider metadata") : {}; return { instrumentId: asString(row.instrument_id, "instrument id"), interval: asString(row.interval, "interval") as MarketCandleRecord["interval"], timestamp: asString(row.timestamp, "timestamp"), ...(typeof metadata.knowledgeAt === "string" ? { knowledgeAt: metadata.knowledgeAt } : {}), open: asString(row.open, "open"), high: asString(row.high, "high"), low: asString(row.low, "low"), close: asString(row.close, "close"), volume: typeof row.volume === "string" ? row.volume : "", currency: typeof metadata.currency === "string" ? metadata.currency : "", provider: (row.provider ?? "yahoo") as MarketCandleRecord["provider"], providerSymbol: typeof row.provider_symbol === "string" ? row.provider_symbol : "", adjustmentMode: "raw", fetchedAt: typeof metadata.fetchedAt === "string" ? metadata.fetchedAt : "" }; }); }
  ...
  getIntervalCoverage(): Array<IntervalCoverageSegment & { instrumentId: string; adjustmentMode: "raw" }> { return (this.database.prepare("select * from interval_coverage order by instrument_id, interval").all() as Row[]).map((row) => ({ instrumentId: asString(row.instrument_id, "instrument id"), interval: asString(row.interval, "interval") as IntervalCoverageSegment["interval"], requestedStart: typeof row.start_timestamp === "string" ? row.start_timestamp : "", requestedEnd: typeof row.end_timestamp === "string" ? row.end_timestamp : "", status: "complete", missingTradingDates: [], adjustmentMode: asString(row.adjustment_mode, "adjustment mode") as "raw" })); }
  ...
  mergeBrowserState(payload: BrowserStatePayload): MigrationReport {
    ...
    const report = withSqliteTransaction(this.database, () => {
      ...
      for (const instrument of payload.instruments) {
        if (this.putInstrument(instrument)) inserted += 1;
        else duplicate += 1;
      }
      ...
      for (const entry of payload.importHistory) { this.putImportHistory(entry); inserted += 1; }
      for (const review of payload.reviews) { if (this.putReviewInTransaction(review)) inserted += 1; else conflict += 1; }
      for (const suggestion of payload.tagSuggestions) { this.putTagSuggestionInTransaction(suggestion); inserted += 1; }
      for (const job of payload.marketDataJobs) { this.putMarketDataJob(job); inserted += 1; }
      for (const candle of payload.dailyCandles) { this.putDailyCandle(candle); inserted += 1; }
      for (const candle of payload.marketCandles) { this.putMarketCandle(candle); inserted += 1; }
      for (const coverage of payload.coverage) { this.putCoverage(coverage); inserted += 1; }
      for (const coverage of payload.intervalCoverage) { this.putIntervalCoverage(coverage); inserted += 1; }
      for (const providerSymbol of payload.providerSymbols) { this.putProviderSymbol(providerSymbol); inserted += 1; }
      for (const [key, value] of Object.entries(payload.settings)) { this.database.prepare("insert into app_settings (key, value_json) values (?, ?) on conflict(key) do update set value_json = excluded.value_json").run(key, json(value, "setting")); inserted += 1; }
    ...
    // Deliberately written after the data transaction commits: a fingerprint
    // never marks a browser payload complete while its data can still roll back.
    this.database.prepare("insert into data_migrations (source_fingerprint, source_client_id, version, status, counts_json, validation_digest, completed_at) values (?, ?, ?, 'complete', ?, ?, current_timestamp)").run(payload.sourceFingerprint, payload.sourceClientId, payload.version, json(report, "migration counts"), report.validationDigest);
    return report;
  }
  ...
  private putDailyCandle(candle: DailyCandleRecord) { this.ensureInstrumentId(candle.instrumentId); for (const field of ["tradingDate", "open", "high", "low", "close", "volume"] as const) asString(candle[field], "daily candle"); this.database.prepare("insert into daily_candles (instrument_id, date, adjustment_mode, open, high, low, close, volume) values (?, ?, ?, ?, ?, ?, ?, ?) on conflict(instrument_id, date, adjustment_mode) do update set open = excluded.open, high = excluded.high, low = excluded.low, close = excluded.close, volume = excluded.volume").run(candle.instrumentId, candle.tradingDate, candle.adjustmentMode, candle.open, candle.high, candle.low, candle.close, candle.volume); this.putProviderSymbol({ instrumentId: candle.instrumentId, provider: candle.provider, providerSymbol: candle.providerSymbol, metadata: { currency: candle.currency, fetchedAt: candle.fetchedAt } }); }
  private putMarketCandle(candle: MarketCandleRecord) { this.ensureInstrumentId(candle.instrumentId); this.database.prepare("insert into market_candles (instrument_id, interval, timestamp, adjustment_mode, open, high, low, close, volume) values (?, ?, ?, ?, ?, ?, ?, ?, ?) on conflict(instrument_id, interval, timestamp, adjustment_mode) do update set open = excluded.open, high = excluded.high, low = excluded.low, close = excluded.close, volume = excluded.volume").run(candle.instrumentId, candle.interval, candle.timestamp, candle.adjustmentMode, candle.open, candle.high, candle.low, candle.close, candle.volume); this.putProviderSymbol({ instrumentId: candle.instrumentId, provider: candle.provider, providerSymbol: candle.providerSymbol, metadata: { currency: candle.currency, fetchedAt: candle.fetchedAt, knowledgeAt: candle.knowledgeAt } }); }
  ...
  private putIntervalCoverage(coverage: IntervalCoverageSegment & { instrumentId: string; adjustmentMode?: "raw" }) { this.ensureInstrumentId(coverage.instrumentId); this.database.prepare("insert into interval_coverage (instrument_id, interval, adjustment_mode, start_timestamp, end_timestamp, updated_at) values (?, ?, ?, ?, ?, current_timestamp) on conflict(instrument_id, interval, adjustment_mode) do update set start_timestamp = excluded.start_timestamp, end_timestamp = excluded.end_timestamp, updated_at = excluded.updated_at").run(coverage.instrumentId, coverage.interval, coverage.adjustmentMode ?? "raw", coverage.actualStart ?? coverage.requestedStart, coverage.actualEnd ?? coverage.requestedEnd); }
  ...
  private validateBrowserState(payload: BrowserStatePayload) { if (!payload || payload.version !== 1 || typeof payload.sourceClientId !== "string" || !payload.sourceClientId || typeof payload.sourceFingerprint !== "string" || !payload.sourceFingerprint) throw new Error("Invalid browser state"); for (const key of ["executions", "importHistory", "instruments", "reviews", "tagSuggestions", "marketDataJobs", "dailyCandles", "marketCandles", "coverage", "intervalCoverage", "providerSymbols"] as const) if (!Array.isArray(payload[key])) throw new Error(`Invalid ${key}`); payload.instruments.forEach(validateInstrument); payload.executions.forEach(validateExecution); payload.reviews.forEach(validateReview); payload.tagSuggestions.forEach(validateTagSuggestion); }
}
