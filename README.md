...
- 富途 XLS/XLSX、富途/Tiger 历史月结单 PDF 和招商证券 PDF 按文件内容自动识别，并在浏览器本地解析
- 独立的“导入 TradingView 模拟交易”入口，支持中文 CNY 回放交易 CSV；识别沪深证券市场和代码，缺失时手动核对，本地解析并持续标注模拟盘
...
可一次选择多份月结单，按文件逐份核对、确认。原件沿用 `YYYY-MM.pdf`，综合账单不拆文件，港股/美股按记录的市场字段组织。

- 先核对模板、来源页、持仓及辅助流水，再确认股票/ETF 分类和保存；无成交月份也可保存证据。
- 行内时区优先，其次原件图例；缺少依据时要求人工提供时区。纽约时间按历史夏令时转换，不使用电脑时区，也不按文件月份硬编码。
- 下单时间不冒充成交秒数；仅日期证据限制在日线/周线复盘。市场自然日期与夜盘业务交易日分开保存。
- 同文件重导可更新解析；跨文件保留同秒多笔成交的数量，账户不同不合并；不同成交的冲突需逐组确认。
- 富途订单主行与分笔成交会归并为一条复盘交易，保留 `source.executionGroup.fills` 作为原始分笔证据；没有明确订单关联的相同时间成交不会被强行合并。
- 新股配售以 `ipo` 辅助事件保留，不伪造普通买入；有数量的配售事件和暗盘/OTC 成交默认锚定到对应交易日的第一根 K 线，原始日期时间仍保留在来源证据中。
- 起始持仓、转入和历史断档不会被伪造成买入；成本或费用不完整时隐藏确定盈亏与路径收益指标。
- 无法读取的字体、未知模板或不守恒的合计会阻断，不能通过勾选警告强行导入。模板覆盖及验收仍以 [设计说明](docs/superpowers/specs/2026-09-07-monthly-statement-import-design.md) 和实际原件核验为准。

本地原件集成审计（不上传文件、不打印账户与逐笔文本）：

```bash
BROKER_CORPUS_ROOT=/path/to/statements npm run test:unit -- app/lib/import/monthly-corpus.test.ts
```

审计通过只代表识别、结构和证据链检查通过，不代表所有逐笔金额均已人工认证。

## 隐私与数据边界

- `trades/` 下的原始券商文件被 Git 忽略，不会进入提交或部署。
- 富途 XLS/XLSX/PDF、Tiger PDF 和招商证券 PDF 均只在浏览器中解析，原始文件不会上传；确认导入后，结构化成交、持仓及辅助流水证据会写入本机 SQLite 服务。
...
- TradingView 当前适配中文 CNY、沪深证券、仅日期的完整进出场配对 CSV。缺失配对、日期顺序有歧义和相反持仓重叠会排除整组并显示诊断；不同内容的重叠导出属于独立运行，不自动拼接。
...
“导入交易文件”支持招商证券普通对账单，显示格式名为 **A股招商银行**。导入沪深 A股和 ETF，排除可转债、港股通及非交易流水；支持独立列和既有合并列布局、按账户核对重叠账单、原始结算金额及费用保存。只有日期的记录会在确认页提示时间精度。

设计与验证见 [A股招商银行导入说明](docs/specs/2026-09-09-cms-a-share-import.md)。
